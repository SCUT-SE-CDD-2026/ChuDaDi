...
* **Facade + Observer + Command 解耦**：`GameAudioService` 作为外观类统一管理 SFX（SoundPool）和 BGM（MediaPlayer）。`AudioHost` composable 通过直接 collect `StateFlow<MatchUiState>` 观察游戏状态变化，`AudioEventDetector` 纯 Kotlin 检测状态差异并封装为 `AudioEvent` 事件对象派发，游戏逻辑层完全无需感知音频系统存在。
* **生命周期安全**：`AudioViewModel` 持有音频服务实例，配置变更时存活。`LifecycleStartEffect` 管理前后台暂停/恢复。
...
| **单例模式 (Singleton)** | 全局唯一的设备管理类，如 `BluetoothConnectionManager`。 |
| **外观模式 (Facade)** | 音频子系统：`GameAudioService` 统一对外 API，内部组合 `SfxPlayer`（SoundPool）和 `BgmPlayer`（MediaPlayer），View 层无需关心底层实现。 |
