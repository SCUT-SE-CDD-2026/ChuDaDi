# Android 蓝牙锄大地游戏技术架构文档 (OOP & MVC 版)
...
└── app/                         # 【唯一的 Android 模块】
    ...
    └── src/main/
        ...
        ├── res/                 # 图片、字符串等资源
        └── java/com/example/chudadi/ # 所有代码的根包
            │
            ├── CDDApp.kt        # 全局 Application 类
            │
            ├── model/game/      # 【M】模型层：纯纯的 Kotlin 数据与规则，不包含任何 Android API
            │   ├── entity/      # 基础实体：Card(牌), Player(玩家), Room(房间)
            │   ├── engine/      # 权威状态机：GameEngine (管理全局手牌、判断输赢)
            │   ├── rule/        # 游戏规则引擎：牌型校验规则 (SingleRule, BombRule)
            │   └── snapshot/    # 面向 UI / 协议的安全状态投影
            │
            ├── ui/              # 【V】视图层：只做 UI 渲染，不写任何游戏规则 (Compose)
            │   ├── home/        # 入口页
            │   ├── game/        # 对局页
            │   ├── result/      # 结算页
            │   ├── theme/       # Compose 主题
