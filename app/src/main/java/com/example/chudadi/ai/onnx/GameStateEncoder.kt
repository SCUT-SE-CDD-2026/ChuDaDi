 ...
 * - 104..112：action_type_one_hot（上一手牌型 one-hot，9）
 * - 113..126：action_length_one_hot（上一手牌张数 one-hot，14）
 * - 127..129：leader_relative_pos（领出者相对位置，3）
 * - 130..171：cards_left（三家剩余牌数 one-hot，42）
 * - 172..327：history（三家历史已出牌 one-hot，156）
 * - 328：is_leader（当前是否自己领出，1）
 * - 329..331：relative_pass_mask（三家是否已 pass，3）
...
class GameStateEncoder {
    ...
    fun encode(match: Match, seatIndex: Int): FloatArray {
        ...
        tensor[offset] = if (match.ruleSet == GameRuleSet.NORTHERN) 1f else 0f
    ...
    /**
     * 上一手牌型编码（9 维，与 RLCard 对齐）：
     * none/single/pair/triple/straight/flush/full_house/four_of_a_kind/straight_flush
     */
     ...
     * 顺序: 下家, 对家, 上家
     *
     * 与 RLCard 语义对齐：追踪当前轮次中、自上一次非 pass 出牌后已 pass 的玩家。
     */
    private fun encodePassMask(
        ...
        for ((i, relPos) in listOf(1, 2, 3).withIndex()) {
            val seatIndex = (currentSeat + relPos) % 4
            if (seatIndex in match.trickState.passedSeatIndices) {
                tensor[offset + i] = 1f
            }
        }
    }
